<template>
	<article :class="[ 'post', 'u-top-spacer-xxxl', { 'post--full': fullPost }]">
		...
		<div :class="fullPost ? 'wrap wrap--slim' : 'post__info'">
			...
			<component 
				:is="fullPost ? 'h1' : 'h2'" 
				:class="[ 'post__heading', { 'u-bottom-spacer-s': fullPost, 'u-bottom-spacer-s': !fullPost }]">
				<g-link 
				v-if="!fullPost"
				:to="type === 'post' ? `blog/${content.slug}` : `projects/${content.slug}`" 
				ref="h-_ink"
			...
			<Button 
				...
				ref="cta_link"
				:href="type === 'post' ? `blog/${content.slug}` : `projects/${content.slug}`"
				icon="arrow-right"
				@mouseover.native="hoverAll"
				@mouseleave.native="leaveAll"
			/>           
			...
			<Content v-if="fullPost" :content="content.content" class="post__content-area u-top-spacer-xxl" />  
